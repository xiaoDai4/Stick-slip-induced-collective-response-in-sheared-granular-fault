# -*- coding: utf-8 -*-
"""
Created on Sat Jul 16 16:13:21 2022
...
# plt.title('Stick, time = %.2f ms ' % ConSV_PlateBottomBeadMiddle_.loc[index_,'Time'], fontsize = 23)
plt.xlabel('$\\mathit{x}$ (mm, stick, time = %d ms)' % ConSV_PlateBottomBeadMiddle_.loc[index_,'Time'], fontsize = 20)
plt.ylabel('$\\mathit{y}$ (mm)', fontsize = 20)
plt.tick_params(labelsize = 17)

plt.savefig('scatter plot of vx(max min is plate motion).svg', dpi=600, format='svg')
plt.show()
...
plt.savefig('scatter plot of vx(extended range).svg', dpi=600, format='svg')
plt.show()
...
plt.rcParams['font.size'] = 17
fig = plt.figure(figsize=(20, 2))
plt.scatter(x1_ - 70,x2_ - 252,c = c_, s = 10, cmap = 'seismic',  vmin = 0, vmax = 0.0005)
cb = plt.colorbar()
...
plt.savefig('scatter plot of v.svg', dpi=600, format='svg')
plt.show()